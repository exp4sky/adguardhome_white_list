...
case $cpu_arch in
        "x86_64" | *"amd64"*)
            docker pull $image/embyserver:4.8.0.80
			;;
        "aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
			if [ "$image" == "amilys" ]; then
                echo "amilys镜像不支持arm架构，现在使用官方镜像"
			fi	
            docker pull emby/embyserver_arm64v8:4.8.0.80
            ;;
        *)
...
docker_exist=$(docker images |grep embyserver |grep 4.8.0.80)
emby_image=$(docker images |grep embyserver |grep 4.8.0.80 |awk '{print $1}')
...
case $cpu_arch in
	"x86_64" | *"amd64"*)
		if [ $mode == "host" ]; then
			docker run -d --name emby -v $media_dir/config:/config -v $media_dir/xiaoya:/media -v /etc/nsswitch.conf:/etc/nsswitch.conf --network=host --add-host="xiaoya.host:$xiaoya_host" $gpu --user 0:0 --restart always $image/embyserver:4.8.0.80
		fi
		if [ $mode == "bridge" ]; then
			docker run -d --name emby -v $media_dir/config:/config -v $media_dir/xiaoya:/media -v /etc/nsswitch.conf:/etc/nsswitch.conf -p 6908:6908 --add-host="xiaoya.host:$xiaoya_host" $gpu --user 0:0 --restart always $image/embyserver:4.8.0.80
		fi
		echo "一键全家桶全部安装完成"
		;;
	"aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
		if [ $mode == "host" ]; then	
	        docker run -d --name emby -v $media_dir/config:/config -v $media_dir/xiaoya:/media -v /etc/nsswitch.conf:/etc/nsswitch.conf --network=host --add-host="xiaoya.host:$xiaoya_host" $gpu --user 0:0 --restart always emby/embyserver_arm64v8:4.8.0.80
		fi
		if [ $mode == "bridge" ]; then	
	        docker run -d --name emby -v $media_dir/config:/config -v $media_dir/xiaoya:/media -v /etc/nsswitch.conf:/etc/nsswitch.conf -p 6908:6908 --add-host="xiaoya.host:$xiaoya_host" $gpu --user 0:0 --restart always emby/embyserver_arm64v8:4.8.0.80
		fi
		echo "一键全家桶全部安装完成"
        ;;
	*)
