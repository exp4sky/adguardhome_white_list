...
if [ $1 ]; then
	...
	case $cpu_arch in
                "x86_64" | *"amd64"*)
                        docker pull emby/embyserver:4.8.0.73
			;;
                "aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
                        docker pull emby/embyserver_arm64v8:4.8.0.73
                        ;;
                *)
	...
	docker_exist=$(docker images |grep emby/embyserver |grep 4.8.0.73)
	...
	case $cpu_arch in
		"x86_64" | *"amd64"*)
			docker run -d --name emby -v $1/config:/config -v $1/xiaoya:/media -v /etc/nsswitch.conf:/etc/nsswitch.conf --network=host --add-host="xiaoya.host:$xiaoya_host" --user 0:0 --restart always emby/embyserver:4.8.0.73
			...
			;;
		"aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
		        docker run -d --name emby -v $1/config:/config -v $1/xiaoya:/media -v /etc/nsswitch.conf:/etc/nsswitch.conf --network=host --add-host="xiaoya.host:$xiaoya_host" --user 0:0 --restart always emby/embyserver_arm64v8:4.8.0.73
			#docker cp /tmp/Emby.Server.Implementations.dll emby:/system/
                        #docker exec -i emby chmod 644 /system/Emby.Server.Implementations.dll
                        #docker restart emby
			echo "一键全家桶全部安装完成"
